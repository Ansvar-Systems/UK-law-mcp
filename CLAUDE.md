# UK Law MCP — Project Guide

## Overview
MCP server providing UK primary legislation via Model Context Protocol. Data sourced from legislation.gov.uk (The National Archives). Strategy B deployment (runtime DB download on Vercel cold start).
...
- **Ingestion**: `scripts/ingest.ts` fetches Atom feeds + Akoma Ntoso XML from legislation.gov.uk
