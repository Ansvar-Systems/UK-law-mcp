...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      COUNT(DISTINCT er.document_id) AS uk_statute_count,
  ...
  if (input.has_uk_implementation !== undefined) {
    sql += input.has_uk_implementation ? ` HAVING uk_statute_count > 0` : ` HAVING uk_statute_count = 0`;
  }
  ...
  interface Row {
    ...
    uk_statute_count: number; primary_implementations: string | null;
  }
  ...
  return {
    results: {
      results: rows.map(r => ({
        ...
        uk_statute_count: r.uk_statute_count,
