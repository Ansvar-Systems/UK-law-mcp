/**
 * get_uk_implementations — Find UK statutes implementing an EU directive/regulation.
 */

import type { Database } from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetUKImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface GetUKImplementationsResult {
  eu_document_id: string;
  eu_title?: string;
...
export async function getUKImplementations(
  db: Database,
  input: GetUKImplementationsInput
): Promise<ToolResponse<GetUKImplementationsResult>> {
