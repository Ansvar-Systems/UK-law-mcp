#!/usr/bin/env tsx
/**
 * Database builder for UK Law MCP server.
 *
 * Builds the SQLite database from seed JSON files in data/seed/.
 * Follows the Finnish Law MCP reference pattern.
 *
...
function buildDatabase(): void {
  ...
  const loadAll = db.transaction(() => {
    for (const file of seedFiles) {
      ...
      insertDoc.run(
        ...
        seed.type ?? 'statute',
        seed.title,
        null, // title_en (UK law is already in English)
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('licence', 'Open Government Licence v3');
  });
  writeMeta();
